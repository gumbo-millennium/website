...
@section('content')
<x-title>
    <h1>Foto's uploaden naar {{ $album->name }}</h1>

    <x-slot name="subtitle">
      Voeg je beste kiekjes toe aan het album!
    </x-slot>
</x-title>

<form method="POST" action="{{ route('gallery.album.upload', $album) }}" class="container py-4">
  @csrf
